...
import { getLocale, setLocale, getAvailableLocales, type Locale } from "./i18n/index.js";
 ...
 * 语言切换器
 */
export function renderLocaleToggle(onLocaleChange: () => void) {
  ...
  return html`
    <div class="locale-toggle">
      ...
      >
        ${locales.map(
          (locale) =>
            html`<option value=${locale.code} ?selected=${locale.code === currentLocale}>
              ${locale.name}
            </option>`,
        )}
      </select>
